...
// Replace lifecycle transform's existing bank/onboarding block instead of adding a duplicate block.
const existingLifecycleBankBlock = `    const hasBankAccount = validateSaudiIBAN(String(formData.bankIban || '').replace(/\\s/g, '').toUpperCase());\n    const hasIqama = formData.nationality !== 'NON_SAUDI' || Boolean(formData.iqamaExpiryDate);\n    const derivedOnboardingStatus = formData.nationality === 'NON_SAUDI' && !hasIqama\n      ? 'WAITING_IQAMA' as const\n      : !hasBankAccount ? 'WAITING_BANK' as const : 'COMPLETE' as const;\n\n`;
const correctedBankBlock = `    const normalizedIban = String(formData.bankIban || '').replace(/\\s/g, '').toUpperCase();\n    if (normalizedIban && !validateSaudiIBAN(normalizedIban)) {\n      alert(language === 'ar' ? 'رقم IBAN المدخل غير صحيح. اتركه فارغًا إذا لم يصدر الحساب البنكي بعد.' : 'The entered IBAN is invalid. Leave it empty if the bank account has not been issued yet.');\n      return;\n    }\n    const hasBankAccount = Boolean(normalizedIban) && validateSaudiIBAN(normalizedIban);\n    const hasIqama = formData.nationality !== 'NON_SAUDI' || Boolean(formData.iqamaExpiryDate);\n    const derivedOnboardingStatus = formData.nationality === 'NON_SAUDI' && !hasIqama\n      ? 'WAITING_IQAMA' as const\n      : !hasBankAccount ? 'WAITING_BANK' as const : 'COMPLETE' as const;\n\n`;
if (source.includes(existingLifecycleBankBlock)) {
  source = source.replace(existingLifecycleBankBlock, correctedBankBlock);
} else if (!source.includes('const normalizedIban =')) {
  throw new Error('Existing lifecycle bank block not found');
}

// Ensure the processed employee stores normalized bank/lifecycle state without duplicating lifecycle fields.
const bankIbanAnchor = `      bankSwiftCode: formData.bankSwiftCode ? formData.bankSwiftCode.trim().toUpperCase() : ''`;
if (!source.includes(bankIbanAnchor)) throw new Error('Processed bank anchor not found');
source = source.replace(bankIbanAnchor, `      bankIban: normalizedIban,\n      bankSwiftCode: formData.bankSwiftCode ? formData.bankSwiftCode.trim().toUpperCase() : ''`);
