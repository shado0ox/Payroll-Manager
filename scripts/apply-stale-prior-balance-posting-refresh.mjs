...
patch('src/components/PayrollRunsView.tsx', source => {
  ...
  if (!source.includes('const priorPeriodTransferred = (employeeId: string, periodMonth: string)')) {
    if (!source.includes(selectedBlock)) throw new Error('Missing selected payment block for stale prior balance fix');
    source = source.replace(selectedBlock, normalizedBlock);
  }

  const createBefore = `    const stillEligible = selectedPaymentItems;\n    if (!stillEligible.length) return;\n    const sequence = (currentRun.paymentBatches?.length || 0) + 1;`;
  const createAfter = `    const normalizedRun = normalizeRunCarriedBalances(currentRun);\n    const stillEligible = normalizedRun.items.filter(item =>\n      selectedPaymentEmployeeIds.includes(item.employeeId) &&\n      (item.entitlementStatus || 'PAYABLE') === 'PAYABLE' &&\n      item.netSalary > 0 &&\n      !committedEmployeeIds.has(item.employeeId)\n    );\n    if (!stillEligible.length) return;\n    const sequence = (normalizedRun.paymentBatches?.length || 0) + 1;`;
  if (source.includes(createBefore)) source = source.replace(createBefore, createAfter);
