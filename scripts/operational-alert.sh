...
if [[ -n "${webhook_url}" ]]; then
  curl --fail-with-body --silent --show-error --max-time 15 -H 'Content-Type: application/json' -d "${payload}" "${webhook_url}" >/dev/null
  delivered=true
fi
...
if [[ -n "${resend_api_key}" && -n "${email_from}" && -n "${alert_emails}" ]]; then
  ...
  curl --fail-with-body --silent --show-error --max-time 15 -H "Authorization: Bearer ${resend_api_key}" -H 'Content-Type: application/json' -d "${email_payload}" https://api.resend.com/emails >/dev/null
  delivered=true
fi
