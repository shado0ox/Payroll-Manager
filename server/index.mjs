...
const isDeveloperUser = user => user?.role === 'ADMIN' && String(user?.username || '').trim().toLowerCase() === 'admin';
...
app.get('/api/admin/payroll-data-repair/scan', auth, async (req,res,next) => {
  try {
...
app.post('/api/admin/payroll-data-repair/repair', auth, writeLimiter, async (req,res,next) => {
  const client = await pool.connect();
  try {
    if (!isDeveloperUser(req.user)) return res.status(403).json({ error:'FORBIDDEN' });
