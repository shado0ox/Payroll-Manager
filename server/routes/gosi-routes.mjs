  ...
  router.get('/gosi/invoices/:id/comparison',auth,async(req,res,next)=>{
    try {
      ...
      const [itemsResult,employeesResult,runResult,assignmentsResult,departmentAssignmentsResult]=await Promise.all([
        ...
        pool.query(`SELECT id,employee_no,payload,is_archived FROM ${q('employees')} WHERE company_id=$1`,[invoice.company_id]),
      ...
      const payrollGosi=payrollItem=>{const subjectWage=money(payrollItem?.gosiSubjectAmount||0),isSaudi=payrollItem?.nationality==='SAUDI',paymentMode=isSaudi?(payrollItem?.saudiGosiPaymentMode||'SHARED'):'EMPLOYER_ONLY';
        const employeeShare=isSaudi?money(subjectWage*Number(payrollItem?.gosiEmployeeRate||0)):0,employerShare=isSaudi?money(subjectWage*Number(payrollItem?.gosiEmployerRate||0)):money(payrollItem?.gosiEmployerShare||0),salaryDeduction=money(payrollItem?.gosiEmployeeShare||0),companyBurden=money(payrollItem?.gosiEmployerShare||0);
        return {expected:{subjectWage,employerShare,employeeShare,total:money(employerShare+employeeShare)},paymentMode,salaryDeduction,companyBurden,companyCoveredEmployeeShare:money(Math.max(0,employeeShare-salaryDeduction))};};
      const comparisons=itemsResult.rows.map(item=>{
        ...
        const payrollValues=payrollGosi(payrollItem),expected=payrollValues.expected;
        let status='MATCHED'; if(!employee) status='INVOICE_ONLY'; else if(employee.is_archived) status='ARCHIVED_EMPLOYEE'; else if(!assignedAccountId) status='UNASSIGNED_ACCOUNT'; else if(assignedAccountId!==invoice.account_id) status='WRONG_ACCOUNT'; else if(!payrollItem) status='MISSING_IN_PAYROLL'; else if(Object.keys(actual).some(key=>Math.abs(actual[key]-expected[key])>0.01)) status='DIFFERENT';
        return { identity:item.identity_number,subscriberName:item.subscriber_name,nationality:item.nationality,employeeId:employee?.id || null,employeeNo:employee?.employee_no || '',employeeName:employee ? text(`${employee.payload?.firstNameAr || ''} ${employee.payload?.lastNameAr || ''}`) : '',department:employee ? text(payrollItem?.department || employee.payload?.department) : '',assignedAccountId:assignedAccountId || null,assignmentSource:employeeOverride?'EMPLOYEE':(assignedAccountId?'DEPARTMENT':null),status,archiveReason:employee?.payload?.archiveReason || employee?.payload?.employmentEndReason || null,archivedAt:employee?.payload?.archivedAt || null,actual,expected,paymentMode:payrollValues.paymentMode,salaryDeduction:payrollValues.salaryDeduction,companyBurden:payrollValues.companyBurden,companyCoveredEmployeeShare:payrollValues.companyCoveredEmployeeShare,
      ...
      for(const employee of employees){if(employee.is_archived)continue;const employeeId=employee.id,payrollItem=payrollByEmployee.get(employeeId),employeeOverride=assignmentByEmployee.get(employeeId),accountId=employeeOverride||assignmentByDepartment.get(payrollItem?.department||employee.payload?.department);if(accountId!==invoice.account_id||invoiceEmployeeIds.has(employeeId)||!payrollItem)continue;
        const payrollValues=payrollGosi(payrollItem),expected=payrollValues.expected;
        comparisons.push({identity:employee.payload?.nationalIdOrIqama || employee.payload?.iqamaNumber || employee.payload?.entryNumber || '',subscriberName:'',nationality:employee.payload?.country || '',employeeId,employeeNo:employee.employee_no,employeeName:text(`${employee.payload?.firstNameAr||''} ${employee.payload?.lastNameAr||''}`),department:text(payrollItem?.department||employee.payload?.department),assignedAccountId:accountId,assignmentSource:employeeOverride?'EMPLOYEE':'DEPARTMENT',status:'PAYROLL_ONLY',actual:{subjectWage:0,employerShare:0,employeeShare:0,total:0},expected,paymentMode:payrollValues.paymentMode,salaryDeduction:payrollValues.salaryDeduction,companyBurden:payrollValues.companyBurden,companyCoveredEmployeeShare:payrollValues.companyCoveredEmployeeShare,difference:{subjectWage:-expected.subjectWage,employerShare:-expected.employerShare,employeeShare:-expected.employeeShare,total:-expected.total}});
      }
