...
test('subscription state expires trials and active subscriptions at their deadline', () => {
  ...
  assert.equal(subscriptionState({ subscription_status:'SUSPENDED' }, now).status, 'EXPIRED');
});
...
test('authorization refreshes valid sessions and blocks expired tenant subscriptions', async () => {
  ...
  const req = { path:'/api/payroll-runs' };
  const res = response();
  ...
  assert.equal(res.body.error, 'SUBSCRIPTION_EXPIRED');
